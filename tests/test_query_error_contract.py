...
class TestEmptyKeyIdReportedAsValueError(QueryContractTestCase):
    def test_empty_id_with_float_or_bool_version_is_value_error(self):
        ...
        # No key is ever sealed: an empty id is judged before any state is
        # looked at, so a float- or bool-valued version on an empty id is an
        # identifier problem (ValueError), never TypeError.
    ...
    def test_empty_id_precedence_identical_on_both_queries(self):
        vault = self.open_vault()
        # Nothing is sealed on purpose: whatever the version argument looks
        # like, both queries classify an empty id the same way, and checking
        # the version type first would be the wrong order.
